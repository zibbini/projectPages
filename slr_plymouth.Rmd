...
Finally, I sought to quantify the area of land that would be flooded due to such sea level rise. In the worst case scenario, we could expect to lose up to ~3.8 km<sup>2</sup> of land by 2300. This is roughly equivalent in area to ~633 football fields. Conversely, in the best case scenario, we could still expect to lose ~1.2 km<sup>2</sup> by 2300. This would still be in the vicinity of roughly 200 footbal fields in area. If we look at projections for this century, we could expect to lose between ~0.4 and ~0.7 km<sup>2</sup>. This would be equivalent to ~67-116 football fields in area. 
...
The processing pipeline for this section is split into pre-processing in bash and QGIS 3.10.0, classification and analysis in R, and finally post-processing of the data in bash. 

#### Pre-processing data

Pre-processing leveraged GDAL and Raster processing tools in QGIS to build the final digital terrain model and remove incomplete data. The final model was aggregated by a factor of 10 (from a 2m DTM) to reduce computation time further down the processing pipeline. Conversion of raster to xyz format was handled with GDAL and `sed`. 

#### Classifying and mapping the DTM

Classification was conducted iteratively, across all RCP models for the 50<sup>th</sup> percentile projection using packages `readr` for importing the DTM, and `foreach`, `parallel` and `doParallel` for parallelising the task. 

Plotting the classified DTM was handled with packages `ggplot2` and my own wrapper function: [`plot.save()`](https://github.com/zibbini/minor_projects/blob/master/SLR_Plymouth/R/plot.save.R). Package `rgdal` and `ggplot2` were used for data preparation of the roads shapefile for the final plot. Finally, package `parallel` was used for parallising the plotting task. 

Please note that this is not the most optimised method for classifying and plotting the DTM. I am currently in the process of updating the code to leverage the `raster` and `rasterVis` packages to speed up such operations. Source code for this section can be found [here](https://github.com/zibbini/minor_projects/tree/master/SLR_Plymouth/R).
